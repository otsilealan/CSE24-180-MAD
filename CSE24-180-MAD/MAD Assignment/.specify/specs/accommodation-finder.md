...
- **FR-003**: System MUST hash passwords using bcrypt (cost factor ≥ 10) before storage; SHA-256 alone is not acceptable
- **FR-004**: System MUST support login authentication with email and password
- **FR-005**: System MUST persist user session indefinitely until the user explicitly logs out; no automatic expiry
...
- **FR-030**: System MUST support provider registration with email, password, and phone number (no studentId required)
- **FR-031**: System MUST allow role selection (Student / Provider) during registration
- **FR-032**: Provider accounts MUST be stored in the same User table with role = "Provider"
...
- **Listing**: Represents accommodation with descriptive info (title, **type**: one of [Single Room, Double Room, Bachelor Flat, 1-Bedroom, 2-Bedroom, Shared House]), pricing (price, deposit), location (Gaborone area), features (**amenities**: multi-select from [WiFi, Water, Electricity, Parking, Furnished, Security, Laundry]), temporal data (availabilityDate), visual (images), and status (Available/Reserved). Related to User (provider)
